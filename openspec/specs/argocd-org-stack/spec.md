...
Defines the per-organisation ArgoCD assets generated by the `argocd-org` Helm chart and the hub-level discovery that turns Camel-pushed org bootstrap manifests into running tenant Applications. All assets are reconciled by the single `openshift-gitops` ArgoCD instance.
## Requirements
