...
The service SHALL expose `GET /api/itsm/groups`, `GET /api/itsm/groups/:id`, `POST /api/itsm/groups`, and `PATCH /api/itsm/groups/:id` — all admin-only — with PATCH body keys restricted to `{ name, fullName, users, forms, repo, clusters }`.
...
The `gdfkube.groups` collection SHALL NOT appear in Debezium's `collection.include.list`. Admin group mutations are not part of the provisioning pipeline.

#### Scenario: docs/04-debezium.md is unchanged by this collection

- **WHEN** the next (Debezium) spec ships
- **THEN** its `collection.include.list` remains `gdfkube.requests,gdfkube.forms` (no `gdfkube.groups`)
