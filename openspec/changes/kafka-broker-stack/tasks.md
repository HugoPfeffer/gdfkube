...
- [x] 1.5 Configure `kafka1` advertised listeners to include both the Docker-internal listener (`kafka1:9092`) and the host listener (`127.0.0.1:9092`) so host-side tools can connect
...
- [x] 3.1 Add `kafka-init` service to `docker-compose.yml` with image `apache/kafka:3.7`, `restart: "no"`, `depends_on` kafka1/kafka2/kafka3 `condition: service_healthy`, bind-mount of `init-topics.sh` at `/scripts/init-topics.sh:ro`, entrypoint running the script against `kafka1:9092`, on `gdfkube-net`
...
- [ ] 5.1 Verify all three brokers come up healthy within 60 seconds via `docker compose up -d kafka1 kafka2 kafka3`
- [ ] 5.2 Verify `kafka-init` exits 0 and all 9 topics are listed via `kafka-topics.sh --list`
- [ ] 5.3 Verify each topic's partition count, RF, retention.ms, min.insync.replicas, and cleanup.policy match the catalog via `kafka-topics.sh --describe` and `kafka-configs.sh --describe`
- [ ] 5.4 Verify idempotent re-run: `docker compose run --rm kafka-init` exits 0 with no config changes
- [ ] 5.5 Verify produce/consume probe via `kafka-console-producer.sh` / `kafka-console-consumer.sh` on `gdfkube.audit` from host using `127.0.0.1:9092`
- [ ] 5.6 Verify single-broker-loss tolerance: stop kafka2, produce/consume on `gdfkube.audit` still succeeds
- [ ] 5.7 Verify cold-start: `docker compose down -v && docker compose up -d` → kafka-init runs cleanly
