  ...
  const submit = (e) => {
    ...
    setTimeout(() => {
      ...
      const newReq = {
        ...
        requestId: "01HQ" + Math.random().toString(36).slice(2, 14).toUpperCase(),
        ...
        status: "approval", stage: 0, progress: 3,
        form: formMeta.id,
        formLabel: formMeta.name,
        justification: vars.justification || vars.description || null,
        waiting: "just now",
        estCost: null,
        policyChecks: [
          { label: "RHACM governance baseline", ok: true },
          { label: "Naming convention (hc-" + orgGuess + "-*)", ok: true },
        ],
      };
      window.GDF_DATA.REQUESTS.unshift(newReq);
      navigate("request-detail", {id: newReq.id, submitted: true});
    }, 700);
  };
